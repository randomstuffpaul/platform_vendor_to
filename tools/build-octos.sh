...
usage() {
    ...
    echo -e "  build-octos.sh [options] device"
    echo ""
    ...
    echo -e "    ./build-octos.sh -c1 sprout4"
    echo -e "${rst}"
    exit 1
}
...
if [ "$opt_only" -eq 1 ]; then
    ...
    echo -e "${bldcya}Starting compilation: ${bldgrn}Building ${bldylw}OctOS $VO${rst}"
    echo ""
...
rm -f "$OUTDIR"/target/product/"$device"/to_*-ota*.zip
